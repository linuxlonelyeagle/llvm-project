...
/// Evaluates the AffineMap map with the given operands by computing both
  ...
  LogicalResult matchAndRewrite(AffineForOp forOp,
    ...
    auto simplifyBound = [&](AffineMap map,
      ...
      for (size_t i = 0, e = map.getNumResults(); i < e; ++i) {
        ...
        // Use value bounds analysis to resolve implicit constants.
    ...
    // Try to simplify the lower bound map
    ...
    // Try to simplify the upper bound map
